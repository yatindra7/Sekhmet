...
def patient():
    ...
    patients = patients if patients is None else [sqlalchemy_row_to_dict(patient) for patient in patients]
...
def physician_id(id):
    ...
    physician = Physician.query.filter_by(EmployeeID = id).first()
    
    # get the patient info from appointment
    
    appointments = Appointment.query.filter_by(Physician = physician)
    appointments = appointments if appointments is None else [sqlalchemy_row_to_dict(appointment) for appointment in appointments]
...
def physician_engagements():
    ...
    for physician in physicians:
        _appointments = Appointment.query.filter_by(Physician = physician)
        _appointments = _appointments if _appointments is None else [sqlalchemy_row_to_dict(appointment) for appointment in _appointments]
